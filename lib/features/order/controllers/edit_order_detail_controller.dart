...
class EditOrderDetailController extends GetxController {
  ...
  Future editCommodity(int indexOriginal, int indexConvert) async {
    ...
    final updateDetail = await _inputAPI.updateInputDetail(
      ...
      orderDetail.commodityListSelected[indexConvert].stock
    );
    ...
    if(updateDetail['ok']) {
      _commoditySelected.value.stock = _addValue.value;
      orderDetail.commodityListSelected[indexConvert].stock = _commoditySelected.value.stock;
      
      return {
        'ok': true, 
        'message': 'Se actualizó correctamente la base de datos'
      };
    }else {      
      return {
        'ok': true, 
  ...
  Future deleteCommodityOrderDetail(int indexOriginal, int indexConvert) async {
    ...
    if(deleteInput['ok']) {
      ...
      return {
        'ok': true, 
        'message': 'Se eliminó correctamente la base de datos'
      };
    }else {
      _addValue.value = 1.0;
      _loading.value = false; 
